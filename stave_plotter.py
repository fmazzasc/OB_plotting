...
class StavePlotter:
    ...
    def __init__(self,
        ...
        bool1 = self.get_data(filename=filename, append=False)
        bool2 = True
        if filename2 != "" and bool1==True:
            bool2 = self.get_data(filename=filename2, append=True)
        
        self.complete_run = bool1 and bool2
    ...
    def get_data(self, filename, append=False):
        ...
        if self.filename.endswith('.gz'):
            ...
            d=np.fromfile(self.filename,dtype=np.int32)

            if len(d)< ALPIDE_ROWS*MAX_LANES*ALPIDE_COLS*CHIP_PER_LANE:
                is_good_run = False
            else:
                d = d.reshape((ALPIDE_ROWS*MAX_LANES,ALPIDE_COLS*CHIP_PER_LANE))
                is_good_run = True

        if not is_good_run:
            print('Run incomplete! Moving to next one')
            return is_good_run
...
class THScanStavePlotter(StavePlotter):
    ...
    def plot_chips(self):
        ...
        for link in self.lane_list:
            ...
            for chipid in range(CHIP_PER_LANE):
                chip=self.data[link*ALPIDE_ROWS:(link+1)*ALPIDE_ROWS,chipid*ALPIDE_COLS:(chipid+1)*ALPIDE_COLS]
                chip_mask = np.logical_and(chip!=0, chip!=300)
                hits[link,chipid]=np.sum(chip[chip_mask])/len(chip[chip_mask])

        # hits = np.ma.masked_where(np.logical_or(hits==0), hits)
        plt.figure()
        vmin = np.min(hits[hits>0]) if len(hits[hits>0])>0 else 0
        plt.imshow(hits, cmap="viridis", vmin=vmin)
        ...
        plt.close()
        if vmin>0:
            return float(np.mean(hits[hits>0])), float(np.std(hits[hits>0]))
        return 0,0
    ...
    def plot_stack(self):
        ...
        plt.clim(0, 10*(self.max_charge+1))
        # plt.clim(vmin=np.min(stack[stack>0]))
...
class FHRateStavePlotter(StavePlotter):
    ...
    def __init__(self, filename, filename2, middle_layer, plot_extension, plot_name, yaml_filename, no_rewrite_masks, n_events=3360000):
        ...
        self.magic = 1
        self.yaml_filename = yaml_filename
        self.no_rewrite_masks=no_rewrite_masks
    ...
    def plot_chips(self):
        ...
        stave = np.zeros(shape=(28,CHIP_PER_LANE))
        for link in self.lane_list:
            chipids = [i for i in range(link2master_chip_id_lut[link], link2master_chip_id_lut[link]+7)]
            for chipid in range(CHIP_PER_LANE):
        ...
        plt.figure()
        plt.imshow(hits, cmap="tab20c", norm=mpl.colors.LogNorm(vmin=1./float(self.n_events)/512/1024, vmax=1.))
        plt.xlabel("Chip ID (inside lane)")
        plt.ylabel("Lane")
        cbar = plt.colorbar()
    ...
    def analyse_hits(self, yaml_filename, debug=False):
        ...
        if not self.no_rewrite_masks:
            ## Double column mask
            with open(f"{self.basename}_bad_dcols.yml", 'w') as f:
                yaml.dump(bad_dcols, f)

            ## Empty chips
            with open(f"{self.basename}_empty_chips.yml", 'w') as f:
                yaml.dump(empty_chips, f)

            ## Noisy pixels
            with open(f"{self.basename}_noise_masks.yml", 'w') as f:
    ...
    def analyse_dcols(self, yaml_filename, debug=False):
        """Mask double columns with a high number of hits
        """
        dcol_hits       = np.zeros(shape=(28*CHIP_PER_LANE,512))
        dcol_hit_pixels = np.zeros(shape=(28*CHIP_PER_LANE,512))
        ...
        mode, count = stats.mode(self.data, axis=None)

        print(f"Median: {np.median(self.data)}")
        print(f"Mode: {mode}")

        data = self.data - int(mode)
